...
The following design will consist will achieve the output of the Load LED and pilot LED that can be seen in table \ref{tab:truthtab}. This consists of defining a "dark" and "light" point for the LDR in the previous section. This is implemented with non inverting Schmidt trigger. The hysterisis band is designed to be 0.2V around the center voltage of 2.5V defined in the previous section ($V_{TU}$=2.6V ,$V_{TL}$=2.4V ).\textbf{The following equations are derived extensively alongside the used circuit in Appendix C}.
...
\begin{center}
	...
	$	V_{REF}=\frac{V_{TL}\times (R_2)}{R_1+R_2}+\frac{V_{H}\times R_1}{R_1+R_2}$
\end{center}
Using the above equations for $V_H=5$ and $V_L=0$ ,$\frac{R_1}{R_2}=\frac{1}{25}$. With $R_2$ chosen as 12k\textohm \ $R_1$ can will be 300k\textohm 

To achieve the one \textbf{on} condition for the load LED in table \ref{tab:truthtab} the \textbf{light condition} is "anded" with the load control as well as the PWM signals  
...
\begin{table}[!htb]
	...
	\begin{tabular}{lrrrr}
		\toprule
		$PWM$&$beatle oad Control$&$Light Condition$&$Load LED$&$Pilot LED$\\
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.38\linewidth]{Figures/A7/lightcondcirc.png}
	\caption{Light condition circuit}
	\label{fig:LDR}
\end{figure}
