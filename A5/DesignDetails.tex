...
\begin{figure}[!htb]
\centering
\includegraphics[scale=0.25]{Figures/FinCirc.png}
...
In the design below the concepts of a op amp voltage comparator is used. A comparator will amplify the difference of the two terminals as can be seen with this equation. $V_{OUT} = A_O(V_+ – V_-)$ \cite{Comp}. The output voltage of a op amp can not go above the positive supply rail or below the negative supply rail. The common mode voltage of a op amp is defined as $V_{CM}=\frac{V_+ +V_-}{2}$ and the differential voltage is defined as $V_{D}=(V_+-V_-)$.\begin{wrapfigure}{l}{0.4\textwidth}
\centering
\includegraphics[scale=0.2]{./Figures/hyst}
\caption{Op-Amp with hysteresis implemented compared with no hysteresis \cite{TexHyst}}
\label{fig:hyst}
\end{wrapfigure}
A problem with the standard comparator op-amp configuration is that if a noisy signal is placed at the input of the op-amp it could cause the op amp output to switch between the two supply rails in a short period of time leading to a signal as can be seen in figure \ref{fig:hyst}. A op amp with hysteresis means that the op amp has positive feedback from the output and this causes the output of the op amp to switch from rail to rail at two voltages $V_{TU}$ and $V_{TL}$ , refer to \ref{fig:schmidt}.
...
The absolute maximum common mode input range of the MCP op amp is from $V_{DD}+0.3$ to $V_{SS}-0.3$. The absolute maximum differential voltage input is $|V_{DD}-V_{SS}|$ \cite{MCP}. The positive supply rail of the op amps in this design will be connected to the 5V regulator and the negative supply rails will be grounded($V_{DD}$ and $V_{SS}$). When designing the input voltages to the op amp it was decided to step the voltages down using voltage division because if the battery terminal (max of 7.2V) is connected and a 0V is connected then the differential voltage maximum will be exceeded. If the 7.2V and a 5V is connected to an op amp the common mode voltage will also be exceeded. The circuit that was built was based off the non inverting Schmidt trigger as can be seen in figure \ref{fig:schmidt}. This configuration compares an input voltage with a constant reference voltage and the output will switch to the positive supply rail at an input voltage of $V_{TU}$ and to the negative supply rail at $V_{TL}$ (refer to \ref{fig:schmidt}). The divided voltage from the 5V regulator is used as a reference voltage because it will remain "constant". The suitable $V_{ref-Schmidt}$ is calculated below and $V_{ref-Schmidt}$ is connected to a voltage follower op amp. The battery terminal is then also stepped down in order for the op amp not to exceed common and differential voltage limits and then it is passed through a voltage following op amp. The voltage follower op amp prevents the positive feedback of the Schmidt trigger from interfering with the input voltage at $V_{in}$ (refer to \ref{fig:schmidt}). The battery voltage is divided to be 4.5V ($\frac{7.2*50}{50+30}$=4.5V) at the op amp terminal using $R_7$ and $R_8$(refer to figure \ref{fig:under}). With a battery terminal voltage of 6.2V the voltage at the op amp will be $V_{TU}=3.875$V and with 6V the voltage will be $V_{TL}=3.75$V. $V_L$ (approx 0.5V) is the low output of the Schmidt trigger and $V_H$ is the high output of the Schmidt trigger(approx 5V). Equations below are for a non-inverting Schmidt trigger  as can be seen in figure \ref{fig:schmidt}. The resistors correspond to figure \ref{fig:under}.

\begin{equation}
    V_{TU}=V_s-V_L\times\frac{R_3}{R_4} 
    \label{eq:vtu}
\end{equation}
\begin{equation}
    V_{TL}=V_s-V_H\times\frac{R_3}{R_4} 
    \label{eq:vtl}
\end{equation}
\begin{equation}
    V_{ref-Schmidt}=\frac{V_s}{1+\frac{R_3}{R_4}}
    \label{eq:ref}
\end{equation}


Solve simultaneously for $\frac{R_3}{R_4} :\ $$V_s= V_H\times\frac{R_3}{R_4} + V_{TL}$\  \ and \  \ $V_s= V_L\times\frac{R_3}{R_4} + V_{TU}$ \ $\frac{R_3}{R_4}=\frac{1}{36}$ \ $V_s$=3.889V. Using $V_s $ and  eq \ref{eq:ref} $V_{ref-Schmidt}$ can be solved to be 3.784V, and the appropriate voltage division for can be set up ($\frac{R_6}{R_6+R_5}$). The resistor magnitudes of the resistors used in voltage divisions were chosen to be big so that little current flows (op amps require very small amounts of current at inputs).
\newline
Maximum common mode voltage after voltage division: $\frac{(\frac{7.2\times50}{80}+3.784)}{2}=4.142V$.
\newline
This is significantly less than the absolute maximum voltage of 5V+0.3V.



\begin{figure}[!htb]
\centering
\includegraphics[scale=0.24]{./Figures/undervoltage}
...
\label{eq:freq}
\end{equation}
To prevent the noise of the ac wall plug from interfering with the measurement the low pass cut off filter is designed to cut off 10Hz before 50Hz to ensure a cleaner signal. The capacitor is chosen to be 100nF. Cutting off at a lower frequency would get rid of more noise however the rise time is dependant on the cut off frequency, $T_r \approx \frac{0.35}{f_c}$. For this reason the cut off frequency  is not designed too low.  As an example the error that a 100mV offset on the input to the ADC will have is calculated as $\frac{100mV}{5V}\times 24=0.48V$

Using eq.\ref{eq:freq} with a chosen capacitor of 100nF the required resistance for a 40Hz cut off is found to be 39.78k\textohm.
...
To achieve the differential amplifier setup from figure \ref{fig:difamp} is used. In this design $R_1$=$R_2$ and $R_3$=$R_4$ \cite{difAmp}. The corresponding equation is as seen in equation \ref{eq:diff}.  $V_2$ will be used as the reference voltage and $V_1$ will be our input from the battery. With this configuration the output will be inverted, but this is not a problem because it can be calibrated accordingly in the arduino software.

\begin{equation}
	V_{out}=\frac{R_3}{R_1}\times(V_{2(ref)}-V_{1(bat)}) 
	\label{eq:diff}
\end{equation}

Using eq.\ref{eq:diff} $V_{2(ref)}$ is solved for simultaneously by setting the equations for the scenario when the battery voltage is 7.5V and 5.7V.

\begin{center}
	 $4.5V=\frac{R_3}{R_1}\times(V_{2(ref)}-3.04V) \ \ \ \ \ \ \ \ 0.5V=\frac{R_3}{R_1}\times(V_{2(ref)}-4V) $\newline 
	
	$\frac{4.5V}{(V_{2(ref)}-3.04V)}=\frac{0.5V}{(V_{2(ref)}-4V)}  \ \ \ \ \ \ \ \ V_{2(ref)}=4.12V \ \ \ \ \ \ \frac{R_3}{R_1}=4.167$\newline
\end{center}


$R_3$ is then chosen to be 112k\textohm  (100k\textohm +12k\textohm in lab resistors). $R_1$ is the calculated to be 26.9k\textohm  (27k\textohm   in lab resistors). Both $V_1$ and $V_2$ need to be divided to a lower voltages 4.12V and four volts, the relevant resistors for the voltage division can be seen in figure \ref{fig:batcirc}.  $V_1$ requires a buffer in between the divided battery voltage and the differential op amp to prevent the negative feedback from interfering with the input voltage. $V_2$ also requires a buffer because otherwise $R_2$ and $R_4$ will the change the effective voltage division and 4.12V will not be achieved. 

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.45\linewidth]{Figures/A6/Batterysig.jpg}
